...
import usePageFocus from './utils/pageFocus';
import usePageVisible from './utils/pageVisible';
...
class Fetch<R, P extends any[]> {
  ...
  constructor(
    ...
    addPollingListener?: (...arg: any[]) => any,
    addRefreshListener?: (...arg: any[]) => any,
  ) {
...
function useAsync<R, P extends any[], U, UU extends U = any>(
  ...
  // 增加两个listener
  const addPollingListener = usePageVisible();
  const addRefreshListener = usePageFocus();
    ...
    if (!manual) {
      ...
      if (Object.keys(fetches).length > 0) {
        ...
        if (
          !(
            staleTime === -1 ||
            new Date().getTime() - cacheStartTime <= staleTime
          )
        ) {
          /* 重新执行所有的 cache */
