...
type PageComponent = React.CElement<
  PageProps,
  React.Component<PageProps, any, any>
>;

function setReconciler() {
  const hostConfig: Partial<
    Reconciler<
      React.FunctionComponent<PageProps> | React.ComponentClass<PageProps>
    >
  > = {
  ...
  if (isBrowser) {
    hostConfig.createPullDownComponent = (
      el,
      _,
      R: typeof React,
      customWrapper,
...
function initNativeComponentEntry(R: typeof React, ReactDOM) {
  ...
  class Entry extends R.Component<Record<any, any>, IEntryState> {
    ...
    mount(Component, compId, getCtx) {
      const isReactComponent = isClassComponent(R, Component);
      const inject = (node?: Instance) =>
        node && injectPageInstance(node, compId);
      const refs = isReactComponent
        ? { ref: inject }
        : {
            forwardedRef: inject,
