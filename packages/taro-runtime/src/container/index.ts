...
container
  .bind<TaroElement>(SERVICE_IDENTIFIER.TaroElement)
  .to(TaroElement)
  .whenTargetNamed(ElementNames.Element);
container
  .bind<TaroDocument>(SERVICE_IDENTIFIER.TaroElement)
  .to(TaroDocument)
  .inSingletonScope()
  .whenTargetNamed(ElementNames.Document);
container
  .bind<TaroRootElement>(SERVICE_IDENTIFIER.TaroElement)
  .to(TaroRootElement)
  .whenTargetNamed(ElementNames.RootElement);
container
  .bind<FormElement>(SERVICE_IDENTIFIER.TaroElement)
  .to(FormElement)
  .whenTargetNamed(ElementNames.FormElement);
container
  .bind<InstanceNamedFactory>(SERVICE_IDENTIFIER.TaroElementFactory)
  .toFactory<TaroElement>((context: interfaces.Context) => {
    return (named: ElementNames) => (nodeName?: string) => {
      const el = context.container.getNamed<TaroElement>(
...
container
  .bind<InstanceFactory<TaroText>>(SERVICE_IDENTIFIER.TaroTextFactory)
  .toFactory<TaroText>((context: interfaces.Context) => {
    return (text: string) => {
      const textNode = context.container.get<TaroText>(
...
container
  .bind<TaroNodeImpl>(SERVICE_IDENTIFIER.TaroNodeImpl)
  .to(TaroNodeImpl)
  .inSingletonScope();
container
  .bind<TaroElementImpl>(SERVICE_IDENTIFIER.TaroElementImpl)
  .to(TaroElementImpl)
  .inSingletonScope();
