...
import { Collapse, Loading, Result } from 'ant-mobile-taro';
  ...
  return (
    <>
      ...
      <DemoBlock title="自定义箭头" padding="0" border="none">
        <Collapse
          defaultActiveKey={['1']}
          arrow={(active) => (active ? <ImageIcon /> : <Camera />)}
        >
          <Collapse.Panel key="1" title="第一项">
            你可以通过 Collapse 的 arrow 属性来控制全部面板的箭头
          </Collapse.Panel>
          <Collapse.Panel key="2" title="第二项" arrow={<MinusCircle />}>
            也可以通过 Collapse.Panel 的 arrow 属性来自定义单个面板的箭头
          </Collapse.Panel>
          <Collapse.Panel
            key="3"
            title="第三项"
            arrow={(active) => (active ? <CheckCircle /> : <CloseCircle />)}
          >
